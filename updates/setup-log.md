...
- `_init` fehlte (newlib erwartet OS). Gefixt mit leerem `_init()`-Stub
...
## M2: Static Library und UART-Transport
...
**STM32 UART (USART2):** PA2/PA3, werkseitig mit ST-LINK verbunden, kein externer Chip nötig. 115200 Baud. Dreischritt: Takt einschalten (GPIOA und USART2 getrennt), Pins auf Alternate Function AF7, USART konfigurieren.

**STM32-Linker-Problem:** `sprintf` (für Coverage-Dump) braucht Heap, Linker-Script hatte keinen definiert. Fehler war `undefined reference to 'end'`. Gefixt mit `end`/`_end`-Symbol am Ende von `.bss`.

---

## M3: Manueller Prototyp End-to-End

Testanwendung mit 3 bis 4 Funktionen, eine bewusst nie aufgerufen (Coverage-Lücke). `coverage_dump_uart.c` (AVR und STM32 getrennt) sendet Bitmap über UART statt printf (zu schwer für Embedded).
...
**Interrupt-sicheres cov_mark():** AVR nutzt `ATOMIC_BLOCK` (util/atomic.h), STM32 nutzt PRIMASK-Register direkt per Inline-Assembler (kein CMSIS-Header nötig, hält Kern portabel). Beides bedingt kompiliert (`__AVR__` und `__arm__`), Kern bleibt für beide Plattformen eine Datei.
...
**AVR:** FreeRTOS von feilipu/avrfreertos (Community-Fork mit Mega2560-Support). Zwei Probleme: Board-Config wollte Timer2 mit externem Quarz nutzen, Board hat aber keinen. Umgestellt auf Timer1. Ausserdem fehlte `timers.h` trotz deaktivierter Timer-Funktion. Zwei Tasks gebaut (Anwendung und Dump-Handler), funktioniert wie erwartet.
...
Reihenfolge im Proposal war STM32 zu AVR, tatsächlich lief es umgekehrt: AVR zuerst komplett durchgezogen, dann STM32 nachgezogen (M1 bis M5 für beide Plattformen). Inhaltlich ist die Portierungsarbeit trotzdem gemacht, nur nicht als eigener Schritt mit diesem Namen.
...
**Wichtige Erkenntnis:** Die ursprüngliche manuelle Instrumentierung aus M2 war eigentlich nur Function Coverage (1 Probe pro Funktion), nicht echte Statement Coverage. Automatisches Tool (`instrument.py`) löst das: findet einzelne Anweisungen, fügt `cov_mark(id)` rückwärts ein (damit sich Positionen nicht verschieben), erzeugt automatisch eine ID zu Datei/Zeile-Map.
...
- [ ] Ordnerstruktur: STM32-Code liegt gesammelt in einem Ordner, nicht wie bei AVR nach Meilenstein aufgeteilt (bewusst so gelassen, um AVR-Code nicht anzufassen)
